...
func testHeader(t *testing.T) {
	defer panicRecover(t, "TestHeader")
	...
	if expHeader != resHeader {
		t.Fatal("Different results", expHeader, resHeader)
	}
}

func testMessage(t *testing.T) {
	defer panicRecover(t, "TestMessage")
	...
	if (exp.Header != res.Header) ||
		(exp.Body.Name != res.Body.Name) ||
		(exp.Body.Age != res.Body.Age) ||
		(bytes.Compare(exp.Body.File, res.Body.File) != 0) {
		t.Fatal("Different results", exp, res)
	}
}
